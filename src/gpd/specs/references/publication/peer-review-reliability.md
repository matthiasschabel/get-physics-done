...
5. **Re-enter review.** After revisions, re-run `gpd:peer-review` for the next round. The next manuscript-changing response round starts only when the prior report and canonical paired response artifacts are present: `${selected_publication_root}/AUTHOR-RESPONSE{round_suffix}.md` plus `${selected_review_root}/REFEREE_RESPONSE{round_suffix}.md`. A completed review without that package stays valid for internal review, accept/no-change outcomes, and fresh clearance reruns; require responses only for existing response rounds or `gpd:respond-to-referees`.
