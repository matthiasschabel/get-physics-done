"""Static guards for provider-free Phase 10 lifecycle behavior tests."""
...
PHASE10_FIXTURE_ROOT = TESTS_ROOT / "fixtures" / "live_audit" / "phase10"
WORKFLOW_ROOT = REPO_ROOT / ".github" / "workflows"

PHASE10_ALLOWED_PATH_PATTERNS = (
    re.compile(r"^tests/adapters/test_(?:live_audit_)?phase10_[^/]+\.py$"),
    re.compile(r"^tests/fixtures/live_audit/phase10/"),
    re.compile(r"^tests/helpers/live_audit_harness/phase10(?:/|[^/]*\.py$)"),
)
PHASE10_ALLOWED_CI_CATEGORIES = {"adapters"}
...
MAX_PHASE10_FIXTURE_FILE_BYTES = 64 * 1024
MAX_PHASE10_FIXTURE_TREE_BYTES = 256 * 1024
...
def _phase10_repo_files() -> tuple[Path, ...]:
    return tuple(path for path in _repo_files() if "phase10" in _relpath(path).casefold())


def _phase10_python_files() -> tuple[Path, ...]:
    return tuple(path for path in _phase10_repo_files() if path.suffix == ".py")


def _phase10_fixture_files() -> tuple[Path, ...]:
    if not PHASE10_FIXTURE_ROOT.exists():
        return ()
    return tuple(sorted(path for path in PHASE10_FIXTURE_ROOT.rglob("*") if path.is_file()))
...
def test_phase10_files_stay_inside_adapter_helper_fixture_boundaries() -> None:
    offenders = [
        _relpath(path)
        for path in _phase10_repo_files()
        if not any(pattern.match(_relpath(path)) for pattern in PHASE10_ALLOWED_PATH_PATTERNS)
    ]
    test_categories = {
    ...
    assert offenders == []
    assert test_categories <= PHASE10_ALLOWED_CI_CATEGORIES
    assert PHASE10_ALLOWED_CI_CATEGORIES <= set(CI_CATEGORY_SHARD_COUNTS)


def test_phase10_python_static_contract_stays_provider_free_in_default_pytest() -> None:
    provider_launch_command_re = _provider_launch_command_re()
    offenders: list[str] = []

    for path in _phase10_python_files():
        ...
        if any(_reads_environment(node) for node in ast.walk(tree)):
...
def test_phase10_adds_no_github_actions_workflow_lane() -> None:
    offenders: list[str] = []
    for path in sorted({*WORKFLOW_ROOT.glob("*.yml"), *WORKFLOW_ROOT.glob("*.yaml")}):
        text = path.read_text(encoding="utf-8")
        relpath = _relpath(path)
        if "phase10" in path.name.casefold():
            offenders.append(f"{relpath}: phase10 workflow file")
        if "phase10" in text.casefold():
            offenders.append(f"{relpath}: phase10 workflow content")

    assert offenders == []


def test_phase10_fixtures_stay_compact_class_only_and_provider_free() -> None:
    offenders: list[str] = []
    total_bytes = 0

    for path in _phase10_fixture_files():
        relpath = _relpath(path)
        size_bytes = path.stat().st_size
        total_bytes += size_bytes
        if size_bytes > MAX_PHASE10_FIXTURE_FILE_BYTES:
            offenders.append(f"{relpath}: {size_bytes} bytes exceeds {MAX_PHASE10_FIXTURE_FILE_BYTES}")
    ...
    if total_bytes > MAX_PHASE10_FIXTURE_TREE_BYTES:
        offenders.append(f"phase10 fixture tree: {total_bytes} bytes exceeds {MAX_PHASE10_FIXTURE_TREE_BYTES}")
