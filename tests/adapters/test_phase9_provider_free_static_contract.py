"""Static guards for provider-free Phase 9 live-audit behavior tests."""
...
PHASE9_FIXTURE_ROOT = TESTS_ROOT / "fixtures" / "live_audit" / "phase9"
WORKFLOW_ROOT = REPO_ROOT / ".github" / "workflows"

PHASE9_ALLOWED_PATH_PATTERNS = (
    re.compile(r"^tests/adapters/test_(?:live_audit_)?phase9_[^/]+\.py$"),
    re.compile(r"^tests/fixtures/live_audit/phase9/"),
    re.compile(r"^tests/helpers/live_audit_harness/phase9(?:/|[^/]*\.py$)"),
)
PHASE9_ALLOWED_CI_CATEGORIES = {"adapters"}
...
MAX_PHASE9_FIXTURE_FILE_BYTES = 64 * 1024
MAX_PHASE9_FIXTURE_TREE_BYTES = 256 * 1024
...
def _phase9_repo_files() -> tuple[Path, ...]:
    return tuple(path for path in _repo_files() if "phase9" in _relpath(path).casefold())


def _phase9_python_files() -> tuple[Path, ...]:
    return tuple(path for path in _phase9_repo_files() if path.suffix == ".py")


def _phase9_fixture_files() -> tuple[Path, ...]:
    if not PHASE9_FIXTURE_ROOT.exists():
        return ()
    return tuple(sorted(path for path in PHASE9_FIXTURE_ROOT.rglob("*") if path.is_file()))
...
def test_phase9_files_stay_inside_adapter_helper_fixture_boundaries() -> None:
    offenders = [
        _relpath(path)
        for path in _phase9_repo_files()
        if not any(pattern.match(_relpath(path)) for pattern in PHASE9_ALLOWED_PATH_PATTERNS)
    ]
    test_categories = {
    ...
    assert offenders == []
    assert test_categories <= PHASE9_ALLOWED_CI_CATEGORIES
    assert PHASE9_ALLOWED_CI_CATEGORIES <= set(CI_CATEGORY_SHARD_COUNTS)


def test_phase9_python_static_contract_stays_provider_free_in_default_pytest() -> None:
    provider_launch_command_re = _provider_launch_command_re()
    offenders: list[str] = []

    for path in _phase9_python_files():
...
def test_phase9_adds_no_github_actions_workflow_lane() -> None:
    offenders: list[str] = []
    for path in sorted(WORKFLOW_ROOT.glob("*.yml")):
        text = path.read_text(encoding="utf-8")
        relpath = _relpath(path)
        if "phase9" in path.name.casefold():
            offenders.append(f"{relpath}: phase9 workflow file")
        if "phase9" in text.casefold():
            offenders.append(f"{relpath}: phase9 workflow content")

    assert offenders == []


def test_phase9_fixtures_stay_compact_class_only_and_provider_free() -> None:
    offenders: list[str] = []
    total_bytes = 0

    for path in _phase9_fixture_files():
        relpath = _relpath(path)
        size_bytes = path.stat().st_size
        total_bytes += size_bytes
        if size_bytes > MAX_PHASE9_FIXTURE_FILE_BYTES:
            offenders.append(f"{relpath}: {size_bytes} bytes exceeds {MAX_PHASE9_FIXTURE_FILE_BYTES}")
    ...
    if total_bytes > MAX_PHASE9_FIXTURE_TREE_BYTES:
        offenders.append(f"phase9 fixture tree: {total_bytes} bytes exceeds {MAX_PHASE9_FIXTURE_TREE_BYTES}")
