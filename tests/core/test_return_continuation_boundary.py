...
def test_checkpoint_intent_is_checkpoint_status_scoped(tmp_path: Path) -> None:
    ...
    checkpoint_validation = validate_gpd_return_markdown(checkpoint_payload)
    completed_validation = validate_gpd_return_markdown(completed_payload)

    assert checkpoint_validation.passed is True
    assert checkpoint_validation.fields["checkpoint_intent"]["checkpoint_reason"] == "pre_fanout"
    assert completed_validation.passed is False
    assert any("status 'completed' does not allow gpd_return field(s): checkpoint_intent" in error for error in completed_validation.errors)


def test_checkpoint_intent_rejects_applicator_owned_resume_fields(tmp_path: Path) -> None:
    _write_project_state(tmp_path)
    ...
    assert any("resume_file is applicator-owned checkpoint_intent field" in error for error in validation.errors)


def test_checkpoint_intent_with_parent_resume_file_applies_bounded_segment(tmp_path: Path) -> None:
    gpd_dir = _write_project_state(tmp_path)
    resume_path = gpd_dir / "phases" / "01-test-phase" / ".continue-here.md"
    resume_path.parent.mkdir(parents=True, exist_ok=True)
    resume_path.write_text("resume\n", encoding="utf-8")
    ...
    result = applicator.apply_child_return_updates(
        tmp_path,
        envelope,
    ...
    bounded_segment = stored_state["continuation"]["bounded_segment"]
    assert bounded_segment["resume_file"] == "GPD/phases/01-test-phase/.continue-here.md"
    assert bounded_segment["phase"] == "01"
    assert bounded_segment["plan"] == "01"
    assert bounded_segment["segment_id"] == "checkpoint-pre_fanout-01-01"
    assert bounded_segment["segment_status"] == "waiting_review"
    assert bounded_segment["checkpoint_reason"] == "pre_fanout"
    assert bounded_segment["waiting_reason"] == "user_review"
    assert bounded_segment["first_result_gate_pending"] is True
    assert bounded_segment["downstream_locked"] is True
...
def test_explicit_bounded_segment_takes_precedence_over_checkpoint_intent(tmp_path: Path) -> None:
    ...
    envelope = GpdReturnEnvelope.model_validate(
        {
            ...
            "continuation_update": {
                "bounded_segment": {
                    ...
                    "phase": "01",
                    "plan": "01",
                    "segment_id": "explicit-segment",
                    "segment_status": "paused",
                    "checkpoint_reason": "explicit",
                },
            },
            "checkpoint_intent": {
                "checkpoint_reason": "intent-only",
                "awaiting": "user_review",
                "first_result_gate_pending": True,
            },
        }
    ...
    assert result.passed is True
    assert stored_state["continuation"]["bounded_segment"]["segment_id"] == "explicit-segment"
    assert stored_state["continuation"]["bounded_segment"]["checkpoint_reason"] == "explicit"
